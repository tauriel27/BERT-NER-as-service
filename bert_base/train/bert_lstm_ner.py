...
from bert_base.train.models import create_model, create_classification_model, InputFeatures, InputExample
                    ...
                    if len(contends) == 0 and len(words) > 0:
                        ...
                        for l, w in zip(labels, words):
                            if len(l) > 0 and len(w) > 0:
                                ...
                                self.labels.add(l)
                                word.append(w)
                        lines.append(['<SEP>'.join(label), '<SEP>'.join(word)])
                        words = []
                        labels = []
                        continue
                if contends.startswith("-DOCSTART-"):
...
def convert_single_example(
    ...
    textlist = example.text.split('<SEP>')
    labellist = example.label.split('<SEP>')
    tokens = []
    labels = []
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        total_loss, logits, trans, pred_ids = create_model(
            args, bert_config, is_training, input_ids, input_mask, segment_ids, label_ids,
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            train_op, lr_tensor = optimization.create_optimizer(
                total_loss, learning_rate, num_train_steps, num_warmup_steps, False)
            hook_dict = {}
            hook_dict['loss'] = total_loss
            hook_dict['global_steps'] = tf.train.get_or_create_global_step()
            hook_dict['lr'] = lr_tensor
            tf.summary.scalar('lr', lr_tensor)
...
def get_last_checkpoint(model_path):
    if not os.path.exists(os.path.join(model_path, 'checkpoint')):
        logger.info(
            'checkpoint file not exits:'.format(
                os.path.join(
                    model_path,
                    'checkpoint')))
        return None
...
def train(args):
    ...
    # tf 新的架构方法，通过定义model_fn
    # 函数，定义模型，然后通过EstimatorAPI进行模型的其他工作，Es就可以控制模型的训练，预测，评估工作等。
    ...
    if args.do_train and args.do_eval:
        ...
        eval_spec = tf.estimator.EvalSpec(input_fn=eval_input_fn, throttle_secs=200)
    ...
    if args.do_predict:
        ...
        def result_to_pair(writer):
            for predict_line, prediction in zip(predict_examples, result):
                idx = 0
                line = ''
                line_token = str(predict_line.text).split('<SEP>')
                label_token = str(predict_line.label).split('<SEP>')
