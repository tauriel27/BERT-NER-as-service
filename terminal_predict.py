...
with graph.as_default():
    ...
    input_ids_p = tf.placeholder(
        tf.int32, [batch_size, args.max_seq_length], name="input_ids")
    input_mask_p = tf.placeholder(
        tf.int32, [batch_size, args.max_seq_length], name="input_mask")

    bert_config = modeling.BertConfig.from_json_file(
        os.path.join(bert_dir, bert_config_file))
    (total_loss,
     logits,
     trans,
...
def predict_online():
    ...
    def convert(line):
        feature = convert_single_example(
            0, line, label_list, args.max_seq_length, tokenizer, 'p')
        ...
        segment_ids = np.reshape(
            [feature.segment_ids], (batch_size, args.max_seq_length))
    ...
    with graph.as_default():
        ...
        while True:
            ...
            #todo: 组合策略
            result = strage_combined_link_org_loc(
                sentence, pred_label_result[0])
            print(
                'time used: {} sec'.format(
...
def strage_combined_link_org_loc(tokens, tags):
    ...
    person, loc, org, kun, tit, wea = eval.get_result(tokens, tags)
    print_output(loc, 'LOC')
    print_output(person, 'PER')
    print_output(org, 'ORG')
    print_output(kun, 'KUN')
    print_output(tit, 'TIT')
    print_output(wea, 'WEA')
...
class Result(object):
    ...
    def get_result(self, tokens, tags, config=None):
        ...
        return self.person, self.loc, self.org, self.kun, self.tit, self.wea
    ...
    def append(self, word, start, end, tag):
        if tag == 'LOC':
            ...
            self.org.append(Pair(word, start, end, 'KUN'))
        elif tag == 'TIT':
            self.org.append(Pair(word, start, end, 'TIT'))
        elif tag == 'WEA':
            self.org.append(Pair(word, start, end, 'WEA'))
        else:
